...
import cSharpQuestions from '../data/data'
import './CsharpQuestions.css'
import axios from 'axios'




function CsharpQuestions() {
  const [currentQuestions,setCurrentQuestion] = useState(0)
    const [showScore,setShowScore] = useState(false)

    
    const [score,setScore] = useState(0)
    const final =cSharpQuestions.cSharpQuestions
    ...
    <div className='app'>
      ...
      {showScore ? (
        <h3 className='score-container'>{score < 8?`Ohps!!! Your score is low, ${score *10} % out of ${final.length *10}%` :
         `Congratulations!!! Your score is ${score *10} % out of ${final.length}`}</h3>
